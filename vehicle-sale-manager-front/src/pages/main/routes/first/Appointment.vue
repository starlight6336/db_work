  ...
  <div class="background-div">
    <el-container class="main-div" v-if="tutorialReady">
      ...
      <el-footer class="btn-container">
        <el-button v-if="canCreateNewConsult" class="start-btn" type="warning" @click="handleStartProcess">开始预约
        </el-button>
        <el-button v-else class="start-btn" type="info" @click="handleCannotContinue">开始预约</el-button>
    ...
    <el-container class="main-div" v-else>
      ...
      <el-footer class="btn-container">
        <el-button class="btn" @click="saveDraft">另存为草稿</el-button>
...
export default {
  ...
  created () {
    this.getId()
    this.tagPool = []
    checkIfCanContinue(this.id).then(res => {
      console.log(res)
      if (res.status === true) {
        if (res.data === false) {
          this.canCreateNewConsult = false
        } else {
          this.canCreateNewConsult = true
        }
      } else {
    ...
    getReservationInfoByID(this.id).then(res => {
      if (res.status === true) {
        ...
        if (res.data.studentInfo != null) {
          this.basicInfo.studentInfo.id = res.data.studentInfo.id
  ...
  methods: {
    ...
    saveDraft () {
      ...
      let json = {
        ...
        studentInfo: {
          id: this.basicInfo.studentInfo.id,
    ...
    submit () {
      ...
      let json = {
        ...
        sex: this.basicInfo.sex,
        birth: this.basicInfo.birth,
        selfEmergency: this.questionInfo.selfemergency,
        expectTime1: expectTime1,
        expectTime2: expectTime2,
        expectTime3: expectTime3,
        studentInfo: {
    ...
    handleCannotContinue () {
      this.$confirm('您存在未完成的预约，故无法开始新的预约，是否跳转到我的预约列表以查看详情?', '不可预约', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
