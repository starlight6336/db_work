<template>
  <div class="background-div">
    <el-header class="header" height="30px">初访员列表</el-header>
      ...
      >
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
                size="mini"
                @click="handleCheck(scope.row.id)">配置初访时间
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!--对齐-->
    <el-dialog title="配置初访时间" :visible.sync="dialogTableVisible" width="80%">
      <el-table :data="gridData" :header-cell-style="{'text-align':'center'}" border="1px">
        ...
        <el-table-column property="periods" label="初访时间" width="800px">
          <template slot-scope="scope">
            <div>
        ...
        <el-dialog
            ...
            append-to-body>
          <el-form>
            <el-form-item label="初访时长" :label-width="formLabelWidth">
              {{ consultDuration }}分钟
            </el-form-item>
            <el-form-item :label-width="formLabelWidth" label="开始时间">
              <el-time-picker v-model="consultStartTime" placeholder="请选择初访时间" :clearable="false" format="HH:mm"
                              value-format="HH:mm">
              </el-time-picker>
            </el-form-item>
          </el-form>
...
<script>
import {_getPreVisitor} from '../../../../api/teacher'
import {
  getPreCheckDuration,
  insertPreCheckSchedule,
  delPreCheckSchedule,
  getPreCheckSchedule
} from '../../../../api/schedule'
...
export default {
  ...
  methods: {
    ...
    handleAddTime () {
      ...
      insertPreCheckSchedule(this.selectedId, submitTime1, submitTime2, parseInt(this.selectedIndex) + 1).then(res => {
        // console.log(res)
        if (res.status === true) {
    ...
    getConsultDuration () {
      return getPreCheckDuration().then(res => {
        // console.log(res)
        this.consultDuration = res.data
      })
    },
    find () {
      return _getPreVisitor(this.searchName).then(res => {
    ...
    handleCheck (id) {
      this.selectedId = id
      getPreCheckSchedule(id).then(res => {
    ...
    parseResponse (resp) {
      ...
      resp.map(item => {
        ...
        // console.log(weekdays[currWeekday - 1].tags)
      })
      return weekdays
    },
    handleClose (i, name, id) {
      delPreCheckSchedule(id).then(res => {
        if (res.data === true) {
          this.$message.success(res.message)
